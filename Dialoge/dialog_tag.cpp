...
Dialog_Tag::Dialog_Tag(QWidget *parent, Project* p, DataBase* db) :
    QDialog(parent),
...
void Dialog_Tag::create_Tags()
{
    ...
    QStringList stringList_Tags = database->get_Tags(string_ProjectID);
...
void Dialog_Tag::slot_TagHighlighted(QListWidgetItem* item)
{
    ...
    for(int i = 0; i < ui->listWidget_Tags->count(); ++i)
    {
        ...
        if(item_Tag->checkState() == Qt::Checked)
            stringList_Temp = stringList_Temp + database->get_ProjectsFromTag(item_Tag->text());
    }
